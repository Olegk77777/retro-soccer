...
//   rig.netScale();                 // ШКАЛА отклика: от касания до пушки
...
//              0.68 м, карман 0.25 / 0.76 / 1.06 м, обратно в поле 1.4 / 3.9 /
//              5.3 м/с, полотно засыпает через 3.6–4.2 с. Мяч во ВСЕХ случаях
//              остаётся в воротах: сетка его убивает, а не отбрасывает
//   netScale — мяч 1 / 2 / 4 / 8 / 16 / 25 / 34 м/с: ход полотна 2.7 / 5.8 /
//              12.5 / 27 / 56 / 87.5 / 118.5 см, вовлечено узлов 9 / 15 / 62 /
//              112 / 183 / 203 / 199 (у задней панели внутренних 261), дрожит
//              1.7 / 1.8 / 2.5 / 2.7 / 3.5 / 3.6 / 3.6 с. Слабое касание
//              ОБЯЗАНО качнуть полотно, сильный удар — натянуть до краёв
...
/**
 * ШКАЛА ОТКЛИКА: одно и то же полотно от лёгкого касания до пушки.
 * Отвечает на прямой вопрос «трясётся ли сетка от слабого касания и
 * натягивается ли от сильного удара» ЧИСЛАМИ, а не впечатлением.
 *
 * Меряем три вещи на каждой скорости: ход полотна (глубина кармана),
 * СКОЛЬКО УЗЛОВ вовлечено (шире куска — сильнее натянуто) и сколько времени
 * полотно живёт после удара.
 */
export function netScale({ speeds = [1.5, 3, 6, 12, 20, 30], y = 0.9 } = {}) {
  const { ball, goals } = dbg();
  const rows = [];
  for (const speed of speeds) {
    const restore = freeze();
    try {
      // Старт вплотную к задней сетке: катящийся мяч тормозится о газон
      // экспонентой и до полотна прокатывает всего v/λ ≈ 1.4·v метра. Первая
      // редакция стенда пускала его с 2.4 м, и строка «1.5 м/с» показывала
      // ноль отклика — не потому, что сетка бетонная, а потому что мяч до неё
      // просто не доезжал.
      const line = CONFIG.field.length / 2;
      ball.reset();
      ball.mesh.position.set(line + CONFIG.goal.depth - 0.8, y, 0);
      ball.vel.set(speed, 0, 0);
      ball.seq = (ball.seq || 0) + 1;

      let carry = 0;
      let moved = 0;      // узлов, сдвинувшихся больше чем на 5 мм
      let peakVel = 0;    // как резко дёрнулось полотно
      let alive = null;
      let touched = false;
      for (let f = 0; f < 480; f++) {
        ball.update(FRAME);
        goals.update(FRAME);
        if (!touched && ball.netContact) touched = true;
        let awake = 0;
        for (const p of goals.panels) {
          ...
          let n = 0;
          for (let i = 0; i < p.count; i++) {
            const o = Math.abs(p.offset[i]);
            if (o > carry) carry = o;
            if (o > 0.005) n++;
            const v = Math.abs(p.velocity[i]);
            if (v > peakVel) peakVel = v;
          }
          if (n > moved) moved = n;
        }
        if (touched && alive === null && awake === 0) alive = f * FRAME;
      }
      rows.push({
        'мяч, м/с': speed,
        'ход полотна, см': +(carry * 100).toFixed(1),
        'узлов вовлечено': moved,
        'пик скорости узла, м/с': +peakVel.toFixed(2),
        'дрожит, с': alive === null ? '> 8' : +alive.toFixed(2),
      });
    } finally {
      restore();
    }
