...
SOCK_Z0, SOCK_Z1 = 0.104, 0.512       # щиколотка / верх отворота гетры
...
TORSO = [
    (1.000, 0.176, 0.114),   # подол футболки — висит ниже пояса
    (1.062, 0.166, 0.106),   # талия, самое узкое место
    (1.135, 0.174, 0.111),
    (1.222, 0.190, 0.119),
    (1.310, 0.203, 0.125),
    (1.392, 0.212, 0.127),   # грудь
    (1.452, 0.213, 0.121),
    (1.500, 0.201, 0.110),   # под плечами
    (1.534, 0.170, 0.096),   # линия плеч / трапеция
    (1.556, 0.103, 0.073),   # основание воротника
]
TORSO_N = 12
...
SLEEVE = [
    ...
    (0.150, 0.093, 1.484),
    (0.225, 0.080, 1.474),
    (0.292, 0.069, 1.466),   # манжета
    (0.305, 0.064, 1.464),   # подворот манжеты внутрь
]
SLEEVE_N = 8
...
ARM = [
    ...
    (0.380, 0.045, 1.464),
    (0.470, 0.042, 1.459),
    (0.512, 0.041, 1.458),   # локоть
    (0.575, 0.043, 1.455),
    (0.700, 0.037, 1.451),
    (0.836, 0.030, 1.446),   # запястье
]
ARM_N = 8
...
HAND = [
    (0.836, 0.034, 0.028, 0.000),
    (0.878, 0.047, 0.031, -0.005),   # ладонь шире запястья
    (0.930, 0.047, 0.030, -0.007),
    (0.972, 0.039, 0.025, -0.009),
    (0.996, 0.021, 0.014, -0.010),   # кончики пальцев
]
HAND_N = 8
# Большой палец: короткий прилив на передней стороне ладони.
THUMB = dict(x0=0.856, x1=0.902, dy=-0.046, dz=-0.004, r0=0.017, r1=0.012, n=6)
...
SHORTS = [
    (1.035, 0.168, 0.108),   # пояс
    (0.968, 0.185, 0.129),
    (0.888, 0.195, 0.139),
    (0.806, 0.191, 0.136),   # подол
    (0.838, 0.148, 0.102),   # подворот внутрь
]
SHORTS_N = 12
...
SOCK = [
    (0.512, 0.063, 0.063),   # верх отворота
    (0.478, 0.058, 0.058),
    (0.430, 0.058, 0.059),
    (0.360, 0.056, 0.058),   # икра
    (0.260, 0.045, 0.047),
    (0.170, 0.037, 0.039),
    (0.104, 0.034, 0.036),   # щиколотка
]
SOCK_N = 8

# Бутса. Считается в СИСТЕМЕ СТОПЫ: начало — щиколотка (кость Foot), ось
# «вперёд» — на носок (кость Toe_End). t — метры вдоль этой оси.
# (t, полуширина, верх по Z, низ по Z)
BOOT = [
    (-0.062, 0.031, 0.060, 0.006),   # пятка
    (-0.030, 0.038, 0.078, 0.002),
    (0.010, 0.042, 0.081, 0.001),    # подъём
    (0.075, 0.045, 0.059, 0.001),
    (0.140, 0.044, 0.043, 0.001),
    (0.205, 0.038, 0.031, 0.002),
    (0.258, 0.024, 0.019, 0.005),    # носок
]
BOOT_N = 8
...
def build_leg(b, sign):
    ...
    def boot_ring(t, hw, ztop, zbot):
        """Сечение бутсы: плоская подошва, скруглённый верх."""
        pts = []
        base = ank + fwd * t
        for k in range(bn):
            a = TAU * k / bn
            # эллипс, у которого низ приплюснут к подошве
            c, s = math.cos(a), math.sin(a)
            zc = (ztop + zbot) * 0.5
            zr = (ztop - zbot) * 0.5
            sq = (abs(c) ** 2.6 + abs(s) ** 2.6) ** (-1.0 / 2.6)
            x = base.x + right.x * hw * c * sq
            y = base.y + right.y * hw * c * sq
            z = zc + zr * s * sq
            pts.append((x, y, z))
        return pts

    boot = [b.ring(boot_ring(t, hw, zt, zb_)) for t, hw, zt, zb_ in BOOT]
    ...
    for i in range(len(boot) - 1):
        t0, t1 = BOOT[i][0], BOOT[i + 1][0]
        fx0 = 1.0 - (t0 - BOOT[0][0]) / (BOOT[-1][0] - BOOT[0][0])
        fx1 = 1.0 - (t1 - BOOT[0][0]) / (BOOT[-1][0] - BOOT[0][0])
        a, bb = boot[i], boot[i + 1]
        for k in range(bn):
            k1 = (k + 1) % bn
            uvs = [uv_in(Z_BOOT, fx0, boot_fy(k)), uv_in(Z_BOOT, fx0, boot_fy(k + 1)),
                   uv_in(Z_BOOT, fx1, boot_fy(k + 1)), uv_in(Z_BOOT, fx1, boot_fy(k))]
            quad = [a[k], a[k1], bb[k1], bb[k]]
            if sign < 0:
                quad = list(reversed(quad))
                uvs = list(reversed(uvs))
            b.face(quad, 'kit', uvs, [(0.0, 0.0)] * 4)
    b.cap(boot[0], 'kit', uv_in(Z_BOOT, 0.98, 0.5), (0.0, 0.0))
    b.cap(boot[-1], 'kit', uv_in(Z_BOOT, 0.02, 0.5), (0.0, 0.0))
