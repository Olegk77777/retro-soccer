...
def flutter_sleeve(x):
    """Свобода рукава. Начинается ТАМ, ГДЕ РУКАВ ВЫШЕЛ ИЗ ТОРСА (|x| ≈ 0.23).

    Прежний порог 0.115 давал ткани свободу ещё внутри торса: рукав у плеча
    шевелился, а торс под ним — нет (там flutter_torso уже ноль). Две отдельные
    замкнутые оболочки начинали скользить друг по другу, и по линии их
    пересечения шёл тёмный пунктир. Проверка `?nocloth` в стенде убирала его
    начисто — то есть виновата была именно ткань, а не атлас и не отсечение
    граней (обе версии проверены и отпали).
    Физически новый порог тоже честнее: рукав натянут на дельтовидной мышце и
    полощется только у манжеты.
    """
    return max(0.0, min(1.0, (abs(x) - 0.230) / 0.115)) ** 1.1
...
# ============================================================================
def _vkey(v):
    """Ключ физической вершины. Округление до 0.1 мм: координаты считаются
    формулами, и две «одинаковые» точки расходятся в последнем разряде."""
...
class Build:
    ...
    def cap(self, ring, mat, uv, flut):
        ...
        Маску ткани здесь можно задавать грубо константой: перед экспортом её
        всё равно сварит weld_cloth() по физической вершине (см. ниже)."""
        self.face(list(ring), mat, [uv] * len(ring), [flut] * len(ring))

    def weld_cloth(self):
        """ОДНА ФИЗИЧЕСКАЯ ВЕРШИНА — ОДНА МАСКА ТКАНИ. Иначе оболочка РВЁТСЯ.

        Плоское затенение расщепляет вершины: у каждой грани свои копии. Маска
        ткани (второй слой UV) едет к вершинному шейдеру `src/cloth.js`, и если
        копии одной точки несут РАЗНЫЕ значения, шейдер разведёт их в разные
        стороны — в оболочке появится щель, а в неё будет виден фон. На экране
        это читается тёмным ПУНКТИРОМ по стыку деталей, и списать его хочется
        на что угодно (мипы атласа, отсечение граней, z-fighting), только не на
        ткань.

        Замер 30.07.2026 на боевой сетке: разъезжались 168 физических точек из
        1148 — подол футболки (крышка несла фазу 0.0 против фазы кольца),
        манжета рукава (крышка 0.00 против свободы 1.00) и подворот шорт
        (0.00 против 0.54). Проверка `?nocloth` в tools/model-preview.html
        убирала пунктир начисто — это и указало на виновника.

        Берём МАКСИМАЛЬНУЮ свободу из копий: крышка на свободном крае обязана
        гулять вместе с тканью, а не держать её гвоздём. Фаза берётся та, что
        пришла с этим максимумом, — у физической вершины она одна по смыслу.
        """
        best = {}
        for idx, _mat, _uvs, flut in self.faces:
            for vi, fl in zip(idx, flut):
                key = _vkey(self.verts[vi])
                cur = best.get(key)
                if cur is None or fl[0] > cur[0]:
                    best[key] = fl
        fixed = 0
        for idx, _mat, _uvs, flut in self.faces:
            for j, vi in enumerate(idx):
                want = best[_vkey(self.verts[vi])]
                if flut[j] != want:
                    flut[j] = want
                    fixed += 1
        print(f'МАСКА ТКАНИ: сварено {fixed} углов граней по {len(best)} точкам')
...
def main():
    ...
    # Маску ткани свариваем ДО bmesh: дальше грани уже разъедутся по копиям
    # вершин, и найти рассинхрон будет не по чему.
    b.weld_cloth()
