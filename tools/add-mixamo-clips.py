...
NEW = [
    ...
    # --- Вратарь, сессия 51 (28.07.2026) ---
    # ЗЕРКАЛЬНЫЙ БРОСОК. В модели лежал ровно ОДИН клип броска, и он играл на
    # оба направления: половина сейвов шла телом в сторону, ПРОТИВОПОЛОЖНУЮ
    # движению. Замер в игре: при броске вправо группа уезжала на z = +2.34, а
    # таз модели — на z = −1.98, то есть картинка расходилась с физикой на
    # 4.2 метра (фидбек Олега 28.07.2026 — «анимация сейвов нереалистичная»).
    # Замер по риггу обоих файлов пака: `diving save` уводит таз на +2.87 по X,
    # `diving save (2)` — на −3.46, то есть это зеркальная пара. Существующий
    # gk_dive идёт в +X, а +X при взгляде в +Z — это ЛЕВО (right = forward × up
    # даёт −X), значит новый клип — бросок ВПРАВО.
    ('goalkeeper diving save (2).fbx', 'gk_dive_r'),
    # Блок корпусом: сейв в упор, когда бросаться некуда и незачем — вратарь
    # раскрывается и принимает мяч телом. Раньше в этой ситуации играл тот же
    # gk_dive, то есть кипер падал вбок от мяча, летящего ему в живот.
    ('goalkeeper body block.fbx',      'gk_block'),
    # Промах («флап»): бросок в пустоту. Нужен, чтобы безнадёжный мяч читался
    # как борьба, а не как поломка, и чтобы ошибка на выходе была ВИДНА.
    ('goalkeeper miss.fbx',            'gk_miss'),
]
