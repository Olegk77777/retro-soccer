...
export function faceTexture(look = {}) {
  ...
  if ((look.hair || 'short') !== 'none') {
    ...
    //
    // ВЕРХ ПОДЛОЖКИ — САМАЯ МАКУШКА (v = 1, то есть первая строка холста), а не
    // полоса шириной с линию роста волос. Раньше подложка кончалась на v = 0.885
    // и верхние 11 % развёртки оставались тоном кожи: стоило шапке хоть немного
    // разойтись с черепом, и на темени вылезало светлое пятно, а тёмным
    // оставался только пояс над лбом — «повязка на голове».
    const yTop = 0;
