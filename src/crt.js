...
const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    vec3 cur0 = vec3(0.0);
    vec3 sL = vec3(0.0);                     // соседи слева и справа: из них
    vec3 sR = vec3(0.0);                     // считается ПЕРЕПАД цветности
    vec3 chAcc = vec3(0.0);
    float yAcc = 0.0;
    float wcSum = 0.0;
    float wySum = 0.0;
    ...
    // Точечный бег: яркость и цветность делили один провод, и остаток
    // поднесущей пролезал в яркость мелкой шахматкой, ПОЛЗУЩЕЙ вбок.
    // Три числа здесь выведены из физики NTSC, а не подобраны, и первая
    // редакция ошиблась во всех трёх.
    //
    // 1) ЧАСТОТА. Поднесущая укладывается в активную строку 188.3 раза
    //    (3.579545 МГц × 52.6 мкс), то есть на нашей сетке 853 ТВ-пикселя
    //    период равен 4.53 ТВ-пикселя = 0.2207 цикла на пиксель. Стояло 0.5 —
    //    это РОВНО частота Найквиста, cos(πn) = (−1)^n: шахматка размером в
    //    пиксель, которая не ползёт, а мерцает на месте и на апскейле даёт муар.
    // 2) ФАЗА ПО ПОЛЮ. Поднесущая кратна ПОЛОВИНЕ строчной частоты, поэтому за
    //    поле фаза уходит на 0.75 цикла, то есть на −90°: последовательность
    //    ЧЕТЫРЁХполевая, и вот от неё берётся медленное ползание. Стояло 0.5 —
    //    двухполевая, шахматка просто мигала инверсией.
    // 3) НА ЧЁМ ОНО ЖИВЁТ. Артефакт рождается на ЦВЕТОВЫХ ПЕРЕХОДАХ, значит
    //    множитель — горизонтальный ПЕРЕПАД цветности, а не её модуль. С
    //    модулем шахматка ложилась на всю насыщенную заливку, и мелко дрожал
    //    весь зелёный газон. Перепад берётся из уже выбранных соседей — даром.
    vec3 cL = sL - vec3(dot(sL, LUMA));
    vec3 cR = sR - vec3(dot(sR, LUMA));
    float chromaEdge = length(cR - cL);
    float phase = uv.x * uScanRes * uAspect * 0.2207 + lineId * 0.5 + uField * 0.25;
    col = max(col + uDotCrawl * chromaEdge * cos(6.2831853 * phase), vec3(0.0));
